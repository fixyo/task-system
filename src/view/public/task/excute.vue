...
<div id="task">
    <div class="indexBody">
        <div class="innerBody">
            <Tabs value="3" :animated="false"  @on-click="detail">
                <TabPane label="任务详情" name="0"></TabPane>
                <TabPane label="我发起的任务" name="1"></TabPane>
                <TabPane label="我接受的任务" name="2"></TabPane>
                <TabPane label="我执行的任务" name="3">
                    <div>
                        <Button class="launchTask" type="primary" style="float:left;" @click="launchTask">发起任务</Button>
                        <div class="notice" style="margin: 0px 0px 10px 90px;">
                            <Icon class="icon" type="md-alert" />
                            <span>您有<span class="num">1</span>个任务方案尚未确认，您有<span class="num">1</span>个任务方案尚未完成，即将<span class="num">超时</span>，请尽快处理!</span>
                        </div>
                    </div>
                    ...
                    <div class="notice">
                        <Icon class="icon" type="md-alert" />
                        <span>提醒：有<span class="num">1</span>个放啊需要确认，有<span class="num">1</span>个方案需要确认，有<span class="num">1</span>个方案需要完成，请尽快处理!</span>
                    </div>
                    <Tabs type="card"  :animated="false"  @on-click="changeInnerTabs">
                        <TabPane :label="'全部('+headeCount.countPlanAll+')'" name="1_1">
                            <Table  :loading="loading"  border ref="selection" :columns="dataTableHeader" :data="dataContent">
                                <template slot-scope="{ row, index }" slot="action">
                                    <Button v-if="row.outButton.confirm !== undefined" type="primary" class="btns" style="margin-right: 5px;">确认</Button>
                                    <Button v-if="row.outButton.finish !== undefined" type="primary" class="btns" style="margin-right: 5px;">完成</Button>
                                    ...
                                    <Poptip placement="bottom" width="100">
                                        ...
                                        <div class="moreAction" slot="content" style="text-aligh:left;">
                                            <p v-if="row.moreButton.confirm !== undefined"class="btns" style="margin-right: 5px;">确认</p>
                                            <p v-if="row.moreButton.finish !== undefined" class="btns" style="margin-right: 5px;">完成</p>
                        ...
                        <TabPane :label="'待确认('+headeCount.countPlanTbc+')'" name="1_2">
                            <Table  :loading="loading"  border ref="selection" :columns="dataTableHeader" :data="dataContent">
                                <template slot-scope="{ row, index }" slot="action">
                                    <Button v-if="row.outButton.confirm!== undefined" type="primary" class="btns" style="margin-right: 5px;">确认</Button>
                                    <Button v-if="row.outButton.finish!== undefined" type="primary" class="btns" style="margin-right: 5px;">完成</Button>
                        ...
                        <TabPane :label="'进行中('+headeCount.countPlanOngoing+')'" name="1_3">
                            <Table  :loading="loading"  border ref="selection" :columns="dataTableHeader" :data="dataContent">
                                <template slot-scope="{ row, index }" slot="action">
                                    <Button v-if="row.outButton.confirm!== undefined" type="primary" class="btns" style="margin-right: 5px;">确认</Button>
                                    <Button v-if="row.outButton.finish!== undefined" type="primary" class="btns" style="margin-right: 5px;">完成</Button>
                        ...
                        <TabPane :label="'已完成('+headeCount.countPlanDone+')'" name="1_6">
                            <Table  :loading="loading"  border ref="selection" :columns="dataTableHeader" :data="dataContent">
                                <template slot-scope="{ row, index }" slot="action">
                                    <Button v-if="row.outButton.confirm!== undefined" type="primary" class="btns" style="margin-right: 5px;">确认</Button>
                                    <Button v-if="row.outButton.finish!== undefined" type="primary" class="btns" style="margin-right: 5px;">完成</Button>
...
export default {
  ...
  data () {
    return {
      ...
      // 内tab任务状态显示的数值
      headeCount: {
        countPlanAll: 0, // 全部显示数值
        countPlanOngoing: 0, // 内tab进行中显示的数值
        countPlanTbc: 0, // 内tab待确认显示的数值
        countPlanDone: 0 // 内tab全部已完成的数值
        ...
        },
        {
          ...
          renderHeader: (h, params) => {
            return (<Poptip placement="bottom">
              ...
              <div slot="content">
                <Input v-model={this.searchHeader.content} clearable placeholder="任务方案" size="small" style="width:110px"></Input>
                <Button class="searchButton" size="small" v-on:click={() => { this.getTableData() }} ><i class = "ivu-icon ivu-icon-ios-search" /></Button>
                <Button type="error" size="small" v-on:click={() => { this.searchHeader.content = '' }}><Icon type="md-close-circle" /></Button>
              </div>
            </Poptip>)
          }
        },
  ...
  methods: {
    ...
    getTableData () {
      ...
      // 内tab数值
      this.$axios({
        url: 'http://192.168.1.103:8080/BzSystem/taskplan/execute/0',
        data: $qs.stringify(),
        method: 'get'
      }).then(resp => {
        if (resp.data.code === 10000) {
          ...
          let { countPlanAll, countPlanDone, countPlanOngoing, countPlanTbc } = responseData
          _self.countPlanAll = countPlanAll
          _self.countPlanDone = countPlanDone
          _self.countPlanOngoing = countPlanOngoing
          _self.countPlanTbc = countPlanTbc
          this.loading = false
        } else {
          this.loading = true
          this.$Message.success(resp.data.msg)
      ...
      this.$axios({
        url: 'http://192.168.1.103:8080/BzSystem/taskplan/execute/' + this.innerTabs + '',
