...
<div id="task">
    <div class="indexBody">
        <div class="innerBody">
            <Tabs value="2" :animated="false"  @on-click="detail">
                <TabPane label="任务详情" name="0"></TabPane>
                <TabPane label="我发起的任务" name="1">
                </TabPane>
                <TabPane label="我接受的任务" name="2">
                        ...
                        <TabPane :label="'全部('+headeCount.countAll+')'" name="1_1">
                            <Table  :loading="loading"  border ref="selection" :columns="dataTableHeader" :data="dataContent">
                                <template slot-scope="{ row, index }" slot="action">
                                    <Button v-if="row.outButton.editTasksPlan!== undefined" type="primary" class="btns" style="margin-right: 5px;">编写方案</Button>
                                    ...
                                    <Poptip placement="bottom" width="100">
                                        ...
                                        <div class="moreAction" slot="content" style="text-aligh:left;">
                                            <p v-if="row.moreButton.editTasksPlan!== undefined"  class="btns" style="margin-right: 5px;">编写方案</p>
...
export default {
  ...
  methods: {
    ...
    getData () {
      // 任务等级 任务类型 任务状态
      this.$axios({
        url: 'http://192.168.1.156:8888/BzSystem/tasks/tasksrender',
        data: $qs.stringify(),
        method: 'get'
      }).then(resp => {
    ...
    getTableData () {
      ...
      // 内tab数值
      this.$axios({
        url: 'http://192.168.1.156:8888/BzSystem/tasks/accept/0',
        data: $qs.stringify(),
        method: 'get'
      }).then(resp => {
        if (resp.data.code === 10000) {
      ...
      this.$axios({
        url: 'http://192.168.1.156:8888/BzSystem/tasks/send/' + this.innerTabs + '',
    ...
    focusTask (index, params) {
      ...
      this.$axios({
        url: 'http://192.168.1.156:8888/BzSystem/tasks/focus',
        data: $qs.stringify(sendData),
        method: 'post'
      }).then(resp => {
        if (resp.data.code === 10000) {
    ...
    insertTasks () {
      ...
      this.$axios({
        url: 'http://192.168.1.156:8888/BzSystem/tasks/inserttasks',
        data: sendData,
        method: 'post'
      }).then(resp => {
